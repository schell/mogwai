...
use futures::SinkExt;
use mogwai_core::{builder::ViewBuilder, patch::ListPatch};
...
impl DomBuilderExt for ViewBuilder<Dom> {
    fn build(self) -> anyhow::Result<Dom> {
        ...
        let mut element = match identity {
            mogwai_core::builder::ViewIdentity::Branch(tag) => Dom::element(&tag, None).unwrap(),
            mogwai_core::builder::ViewIdentity::NamespacedBranch(tag, ns) => {
                Dom::element(&tag, Some(&ns)).unwrap()
            }
            mogwai_core::builder::ViewIdentity::Leaf(text) => Dom::text(&text).unwrap(),
        };

        use mogwai_core::builder::exhaust;
...
#[cfg(test)]
pub(crate) mod test {
    fn sendable<T: mogwai::constraints::SendConstraints>() {}
